...
scrivi("<i>Quelli che seguono sono inevece gli ordini che il Palladio ha voluto potessi addirit"
...
// qua devo mettere un'altra query x gli ordini delle citt�...
$sqlCitta  = "SELECT DISTINCT o.id_ord,o.nome_veloce,o.m_fileImmagineTn,o.sovrano,o.citt� "
...
while ($row=mysql_fetch_array($res0)) 	{
	...
	while ($rsCitta=mysql_fetch_array($resCitta))
		{// x ogni ordine della citt� i-ma... stampa un bel link e magari un bel thumbnail!!!!!
...
//$nickname=String(getUtente())
 ...
 #if (recSet.EOF)
#	 scrivi("<h4>Non possiedi goliardi tuoi!</h4>");
	...
	scrivi("<table ><center>\n");
		...
		trtd();
		freccizzaFrase("Nuovo");
		tdtd();
		formbegin("nuovogoliarda.php");
		formtextBr("dinomegoliardico","NOME (obbligatorio)");
		if (isGuest())

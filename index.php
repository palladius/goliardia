...
$VISUALIZZA_MESSAGGIO_OCCASIONALE 	= 1;
...
scrivi("<center><br>");
scrivi(getFoto2($paz_foto."primapagina/".$FOTOPRIMAPAGINA,"foto prima pagina",500,$paz_foto."serie/matrice.jpg",100));
scrivi("<br/> <b>Fig. A</b>: ");
...
// SBUR USER
$result=mysql_query("select count(*) as N from loginz where m_bguest=0");
...
// totali
$result=mysql_query("select count(*) as N from loginz");
#while ($row = mysql_fetch_array($result)) {scrivib($row[0]); 	}
while ($row = mysql_fetch_row($result)) {scrivib($row[0]); 	}

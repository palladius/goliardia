...
//PROMEMORIA: crea una pagina di CITTA e regioni che si linkano a vicenda... regione di altre nazioni � NAZIONE, compresa ITALIA...
...
?>
<h1>
	<?php  echo  ($m_bCittaNostrana ? "Citt� (citta)" : "Nazioni")?>
</h1>
<?php 
tabled();
...
if (mysql_num_rows($res2)==0)
	{scrivib(rossone("ATTENZIONE!!! La tua provincia (ora ha valore '<i>".Session("provincia")."</i>') non esiste nel database. Vai nella sezione UTENTE, in basso, e correggila con qualcosa di appropriato; magari in futuro contatta il webmaster affinch� aggiunga la citt�/nazione in cui ti trovi adesso..."));
	 bona();
	}


//if ($ISPAL) echo rosso("la citt� esiste? rcord matchanti: <b>".mysql_num_rows($res2)."</b>==1?!?");
...
function reportCittaScelta($citta)
{
 global $DIM_ICONCINA,$SERIOSTRING,$ISSERIO;
	...
	$res= mysql_query("select m_fileImmagineTn as _fotoordine, id_ord as _linkOrd,nome_veloce, m_bInSonno as _inSonno , sovrano as _sovrano,m_nPrecedenzaCittadina AS preced FROM ordini WHERE citt� like '$citta' $SERIOSTRING AND m_bserio=1 order by _inSonno asc, sovrano desc, nome_veloce asc");
	...
	$res= mysql_query("select m_fileImmagineTn as _fotoordine, id_ord as _linkOrd,nome_veloce, m_bInSonno as _inSonno , sovrano as _sovrano $STRADMIN FROM ordini WHERE citt� like '$citta' $SERIOSTRING AND m_bserio=1 ORDER BY sovrano desc, m_nPrecedenzaCittadina asc, nome_veloce asc");
	...
	$res=mysql_query("select * from ordini o, linkz l where o.citt�='$citta' AND o.id_ord=l.id_oggettopuntato AND l.tipo = 'ordini'");
...
scrivi("x ric: metti dopo SOLO citt� estere e nella query metti nome come _nazione, e nel viusalizzatore generico metti _nazione con bandierine di girarrostus...");
tableEnd();
...
if (isadmin())
	{echo h2("osserva i colori feluca x citt� (<b>$citta</b>):");

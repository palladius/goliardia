...
function scriviReport_Utente($rs,$linkami,$ii) {
	...
	if ($ISPAL) { // solo admin vip leggono la mail.
		...
		tdtd();
	} else if (isAdmin() && $rs["m_bGuest"] == 1) { // guest
		$stessaprovincia = (strtolower($rs["provincia"]) == strtolower(Session("provincia")));
		if ($stessaprovincia) {
			formBegin("pag_messaggi.php");
			formhidden("id_login",$rs["ID_LOGIN"]); //so che è sporco, ma x minimizzare è modifiche
			...
			scrivib("province diverse, admin caro...");
		}
	}
	...
	scriviCoppia("GPÆ",$rs["m_nPX"]);  // che cazzo era questo? Devo fare git history. Dai log sembra fosse questo (!!): Æ
...
// git log -L '/getFotoIcona/,+1:funzioni.php'
function getFotoIcona($paz,$h,$alt) {
	if (Session("antiprof"))
		return "PHPUFO"; // originalmente era "PHPUF?" e non sapro' mai cosa fosse il carattere speciale..
	...
	if (!isset($fotosua))
		{$lasciaPerdere=TRUE;
		 scrivi("WARNING!!! <b>".$rs["m_sNome"]."</B> non è foto-dotat$desinenza! Non te l$desinenza faccio votare. "
			...
			."cca qui</a>) che picchierò chi si è permesso di rendere utente questo foto-non-dotato. Grazie.");
		}
	}
...
scrivi("<tr><td><center>");
 ...
 formtext("note","perchè bisogna condividere la propria felicità con altri ogni tanto");
 popolaComboUtentiRegistrati("id_login");
	...
	while(list($chiave,$valore)=each($_POST)) {
		...
		if ($TIPO=="data")
		 	if ($valore == "") { //QWERTY se la data è nulla inserisci un valore nullo secondo me...
				scrivid("[NON MOVONEXT né brekko!!!]</td></tr>");
				$skippa=TRUE;
			}
		if (! $skippa) 	{
			$valore=getNuovoValoreByTipo($valore,$TIPO);
